...
export const getGalleryOrientation = (image: {
  sanityImage?: { asset?: { metadata?: { dimensions?: { width?: number; height?: number } | null } | null } | null } | null;
  image?: { width?: number; height?: number } | null;
} | null): GalleryOrientation => {
  const sanityDimensions = image?.sanityImage?.asset?.metadata?.dimensions ?? null;
  const width = sanityDimensions?.width ?? image?.image?.width ?? null;
  const height = sanityDimensions?.height ?? image?.image?.height ?? null;
...
export const getGalleryLayoutClasses = (
  ...
  const columnStart = getGalleryColumnStart(images, index, startColumn) as keyof typeof galleryColumnStartClasses;
  return [aspectClass, spanClass, galleryColumnStartClasses[columnStart] ?? galleryColumnStartClasses[1]];
};

export const getGalleryImageDimensions = (image: Parameters<typeof getGalleryOrientation>[0]) =>
  getGalleryOrientation(image) === "portrait" ? { width: 825, height: 1100 } : { width: 1100, height: 825 };

export const getGallerySizes = (image: Parameters<typeof getGalleryOrientation>[0]) =>
  getGalleryOrientation(image) === "portrait" ? "(min-width: 1024px) 20vw, 100vw" : "(min-width: 1024px) 40vw, 100vw";
